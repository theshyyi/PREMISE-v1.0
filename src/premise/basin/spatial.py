...
def _optional_geo_dependencies():
    try:
        import geopandas as gpd
        import regionmask
    except ImportError as exc:
        raise ImportError(
            "clip_with_vector requires optional geospatial dependencies. "
            "Install them with: pip install -e '.[geo]'"
        ) from exc
    return gpd, regionmask


def _ensure_geo_crs(gdf):
    if gdf.crs is None:
...
def clip_with_vector(
    ...
    *,
    ...
    gpd, regionmask = _optional_geo_dependencies()
