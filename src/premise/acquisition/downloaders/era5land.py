...
class ERA5LandMonthlyDownloader(BaseDownloader):
    """
    ERA5-Land monthly means downloader using CDS API.

    Current implementation targets:
    - dataset: reanalysis-era5-land-monthly-means
    - output: NetCDF by default
    ...
    def download(self, request: DownloadRequest) -> list[Path]:
        if request.source_key.lower() != "era5land_monthly":
            raise ValueError(f"Unsupported source_key for ERA5LandMonthlyDownloader: {request.source_key}")
        ...
        years, months = self._expand_year_months(request.start_date, request.end_date)
        ...
        recorder = AcquisitionRunRecorder(
            ...
            extra_metadata={
                ...
                "data_format": request.format_preference or "netcdf",
            },
        )

        downloaded_files: list[Path] = []

        # one request -> one output file
        ...
        cds_request = {
            ...
            "year": years,
            "month": months,
            "data_format": (request.format_preference or "netcdf").lower(),
        }

        # Optional area subset
        # CDS commonly uses [north, west, south, east]
    ...
    @staticmethod
    def _expand_year_months(start_date: str, end_date: str) -> tuple[list[str], list[str]]:
    ...
    def _build_output_filename(request: DownloadRequest) -> str:
        ...
        ext = "nc" if (request.format_preference or "netcdf").lower() == "netcdf" else "grib"
        return f"era5land_monthly_{var_part}_{start_part}_{end_part}.{ext}"